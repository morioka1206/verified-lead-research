...
別業種へ広げる前に、[業種・サイト別の難しさと実地メモ](skills/verifying-sales-leads/references/industry-risks.md)も確認してください。対応済みの問題、まだ難しい対象、新業種を10社で試す手順を記録しています。
...
- `leads.xlsx`：判定欄などを入れない、営業にそのまま使える見やすい表
- `leads.csv`：他のシステムへ渡しやすい日本語データ
- `audit.json`：全候補の判定、公式サイトの原文証拠、証拠URL、確認日時

リスト完成後に、品質チェックをするか選べます。既定は10社の抜き取り確認です。確認用の表は営業リストと分け、Googleスプレッドシートまたは `quality-review.xlsx` から選びます。Google Sheetsなら複数人で同時に確認でき、Excelなら手元やオフラインで確認できます。共有相手や公開範囲は自動変更しません。

確認する10社は先頭10社ではありません。判断が難しい会社と、異なる会社種別の代表例を混ぜます。10社中9社以上が正しければ簡易チェックは合格です。8社以下なら20社または全件へ確認を広げます。これは全件の正確さを証明する検査ではなく、大きな問題を早く見つけるための抜き取り検査です。
...
5. AIが対象企業として `accepted` と判定し、3種類すべてがそろい、除外条件を1項目ずつ確認した会社だけを合格にする

分からない会社は無理に合格にせず、`review` または `blocked` にします。会社名、URL、メールアドレスを想像で補いません。

証拠確認プログラムは、証拠不足や取得不能を見つけた時に判定を下げられます。一方、AIが競合・対象外などの理由で `review` または `rejected` にした企業を、証拠が3種類あるという理由だけで `accepted` へ格上げすることはありません。

たとえば「卸・販売会社を探し、自社製造メーカーは除外」という案件では、販売の証拠があってもメーカー該当の確認を省略できません。除外条件の確認がない会社や、製造と販売の境界が曖昧な会社は `review` になります。
...
- クロール結果は1社ごとに保存します。途中で通信エラーや中断が起きても、それまでに確認した会社は残り、同じコマンドを再実行すると未完了の会社から再開します。
...
各バッチの完了後に `leads.xlsx`、`leads.csv`、`audit.json`、`summary.json` が更新されます。`leads.xlsx` は営業用のきれいな一覧です。品質確認を選んだ場合だけ、別に `quality-review.xlsx` またはGoogleスプレッドシートを作ります。候補上限まで確認しても目標に届かない場合は、品質を下げずに不足数を報告します。
